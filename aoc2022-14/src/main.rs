...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let grid_size = ((x_max - x_min) as usize + 1, (y_max - y_min + 1) as usize);
    ...
    loop {
        ...
        if grid[[spawn.0, spawn.1 + 1]] != GridState::Air {
            break;
        } else {
            units += 1;
            grid[[spawn.0, spawn.1 + 1]] = GridState::Sand(SandState::Moving);
        }
